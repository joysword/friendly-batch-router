...
CmGm.sendRequest = function (request, LLid, query_num, travelType) {
	var i, print_results;
		...
		if (status == google.maps.DirectionsStatus.OK) {
			...
			if ($('input[name=outputformat]:checked').val() === 'e') {
				print_results =
					...
					theleg.steps.length + spacer + travelType;	//the number of instruction steps / complexity?.
			} else {
				print_results =
					LLid + spacer +
					theleg.start_location.lat() + spacer +
